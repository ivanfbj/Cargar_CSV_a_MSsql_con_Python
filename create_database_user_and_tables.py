...
if __name__ == '__main__':
    ...
    try:
        ...
        if row_table1 is None:
            ...
            ''')
            print(f'La tabla "DepartmentTest" ha sido creada exitosamente en la base de datos "{database_name}".')
        if row_table1 is not None:
            print(f'La tabla "DepartmentTest" ya existe en la base de datos "{database_name}".')

        # Consulta para validar si existe la tablas en la base de datos
        cursor.execute(f'''
            SELECT TABLE_NAME
                FROM {database_name}.INFORMATION_SCHEMA.TABLES
                WHERE TABLE_CATALOG = '{database_name}' AND TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'DepartmentTest_bulkInsert';''')
        row_table2 = cursor.fetchone()

        # Si la tabla no existe la tablas en base de datos se creará
        if row_table2 is None:
            print(f'NO EXISTE la tabla "DepartmentTest_bulkInsert".')
            cursor.execute(f'''
                USE {database_name}
            ...
            ''')
            print(f'La tabla "DepartmentTest_bulkInsert" ha sido creada exitosamente en la base de datos {database_name}')
        if row_table2 is not None:
            print(f'La tabla "DepartmentTest_bulkInsert" ya existe en la base de datos "{database_name}".')
